...
mod honest {
    ...
    fn parse_args() -> u16 {
        let matches = App::new("tofnd")
            ...
            .arg(Arg::with_name("port").required(true).default_value("50051"))
            .get_matches();
...
pub mod malicious {
    ...
    pub struct Config {
        pub port: u16,
        // malicious field is only for malicious feature
        pub behaviour: MaliciousType,
    }

    impl Config {
        pub fn new() -> Config {
            let (port, malicious) = parse_args();
            Config {
                port,
                behaviour: malicious,
            }
    ...
    fn parse_args() -> (u16, MaliciousType) {
