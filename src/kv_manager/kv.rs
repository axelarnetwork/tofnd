    ...
    V: Debug + Send + Sync + Serialize + DeserializeOwned,
{
    ...
    pub fn new(root_path: &str, kv_name: &str, entropy: &Entropy) -> KvResult<Self> {
        ...
        Self::with_db_name(kv_path, entropy)
    }
    ...
    pub fn with_db_name(full_db_name: String, entropy: &Entropy) -> KvResult<Self> {
        ...
        let kv = get_kv_store(&full_db_name, entropy)?;
...
pub fn get_kv_store(
    ...
    // create/open DB
    let kv = encrypted_sled::open(db_name, entropy)?;
